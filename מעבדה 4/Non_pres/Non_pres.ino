    ...
    case (Transmit): {                                             // Case 1: Transmit
        if (sending == 0) {
          if (checkLine() == 1) {
            ...
            Serial.println();
            startPackage(payload, payload_size);                 // Start the package sending and sample current time, flip sending flag ON
            prev_time = millis();
            sending = 1;
          }
          else {                                                 //  line is busy collision occured: inc. failed packets, enter collision func, then send package again
            failed_packets++;
            cd_func();
            break;
          }
